...
function isChatReference(value: unknown): value is ChatReference { return isChatReferenceBase(value) && isNullableNumber(value.archivedAt); }
...
function isLocalNote(value: unknown): value is LocalNote { return isLocalNoteBase(value) && isNullableNumber(value.archivedAt); }
...
function hasWorkspaceEnvelope(value: Record<string, unknown>): boolean {
  return isString(value.id) && isString(value.name) && Array.isArray(value.folders) && value.folders.every(isFolder) && Array.isArray(value.manualEdges) && value.manualEdges.every(isManualEdge) && Array.isArray(value.tabs) && value.tabs.every(isTab) && isString(value.activeTabId) && isPanelLayout(value.layout) && isNumber(value.updatedAt);
}

export function isWorkspaceSnapshot(value: unknown): value is WorkspaceSnapshot {
  const structurallyValid = isRecord(value) && value.schemaVersion === WORKSPACE_SCHEMA_VERSION && hasWorkspaceEnvelope(value) && Array.isArray(value.chatRefs) && value.chatRefs.every(isChatReference) && Array.isArray(value.notes) && value.notes.every(isLocalNote);
...
function migrateV1Workspace(value: Record<string, unknown>): WorkspaceSnapshot | null {
  if (value.schemaVersion !== 1 || !hasWorkspaceEnvelope(value) || !Array.isArray(value.chatRefs) || !value.chatRefs.every(isChatReferenceBase) || !Array.isArray(value.notes) || !value.notes.every(isLocalNoteBase)) return null;
...
export function importWorkspaceJson(json: string): WorkspaceSnapshot {
  ...
  if (isRecord(parsed) && parsed.schemaVersion !== undefined) {
    if (parsed.schemaVersion === 1 || parsed.schemaVersion === WORKSPACE_SCHEMA_VERSION) throw new Error('Invalid workspace payload.');
