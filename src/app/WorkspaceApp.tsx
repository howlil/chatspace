...
export function WorkspaceApp({ repository, currentUrl = () => window.location.href, navigate = (url) => window.location.assign(url), downloadText = defaultDownloadText, bridge, requestBridgePermission = requestLocalBridgePermission }: WorkspaceAppProps) {
  ...
  function saveCurrentChat(): void {
    ...
    if (existing !== undefined) { openTab(chatTab(existing)); setStatus('Conversation reference is already saved.'); return; }
  ...
  function confirmSaveCurrentChat(input: SaveConversationInput): void {
    ...
    if (existing !== undefined) { setSaveDialogTarget(null); openTab(chatTab(existing)); setStatus('Conversation reference is already saved.'); return; }
    const now = Date.now();
  ...
  function openSavedChat(chat: ChatReference): void { openTab(chatTab(chat)); dispatch({ type: 'chat/update', chat, now: Date.now() }); navigateToChatGptTarget(chat.target, navigate); }
  ...
  function openGraphNode(node: GraphNode): void {
    if (node.kind === 'chat') { const chat = workspace.chatRefs.find((item) => item.id === node.entityId); if (chat !== undefined) openSavedChat(chat); return; }
    if (node.kind === 'note') { const note = workspace.notes.find((item) => item.id === node.entityId); if (note !== undefined) openNote(note); return; }
    if (node.kind === 'folder') { setSelectedFolderId(node.entityId); openHome(); return; }
    openHome();
  }
  ...
  async function importBackup(json: string): Promise<void> {
    ...
    try { await workspaceRepository.save(snapshot); dispatch({ type: 'workspace/replace', snapshot }); setPersistenceError(null); setRecoveryJson(null); setPersistenceState('ready'); setStatus('Backup imported.'); }
    catch (error) { setPersistenceError(`Import could not be saved. ${messageFromError(error)}`); setPersistenceState('blocked'); throw error; }
  }

  async function resetLocalData(): Promise<void> {
    try { await workspaceRepository.clear(); const reset = createInitialWorkspace(); dispatch({ type: 'workspace/replace', snapshot: reset }); setSelectedFolderId(null); setPersistenceError(null); setRecoveryJson(null); setPersistenceState('ready'); setStatus('Local workspace reset.'); }
    catch (error) { setPersistenceError(`Reset failed. ${messageFromError(error)}`); setPersistenceState('blocked'); throw error; }
  }

  async function connectBridge(token: string): Promise<void> {
    setBridgeState('connecting'); setBridgeMessage(null);
    try { const granted = await requestBridgePermission(); if (!granted) throw new Error('Localhost bridge permission was not granted.'); await vaultBridge.health(token); setBridgeToken(token); setBridgeState('connected'); setBridgeMessage(null); }
    catch (error) { setBridgeToken(null); setBridgeState('error'); setBridgeMessage(messageFromError(error)); throw error; }
  }
  ...
  async function syncNoteToVault(note: LocalNote): Promise<void> {
    if (bridgeToken === null) { setBridgeMessage('Connect the local vault bridge before syncing a note.'); openSettings(); return; }
    try { const result = await vaultBridge.writeNote(bridgeToken, note); setStatus(`Synced note to ${result.path}.`); setBridgeMessage(null); }
    catch (error) { setBridgeState('error'); setBridgeMessage(messageFromError(error)); }
  }
  ...
  const commands: WorkspaceCommand[] = [
    ...
    { id: 'folder-create', label: 'Create folder', run: addFolder }, { id: 'note-create', label: 'Create note', run: addNote }, { id: 'chat-save', label: 'Save current chat', run: saveCurrentChat }, { id: 'graph-open', label: 'Open graph', run: openGraph }, { id: 'settings-open', label: 'Open settings', run: openSettings }, { id: 'home-open', label: 'Open home', run: openHome },
  ];
  ...
  else if (activeChat !== undefined) surfaceContent = <section className="workspace-home"><strong>{activeChat.label}</strong><p>{activeChat.target}</p><button type="button" onClick={()=>openSavedChat(activeChat)}>Open in ChatGPT</button></section>;
