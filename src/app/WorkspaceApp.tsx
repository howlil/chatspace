...
export function WorkspaceApp({ repository, currentUrl = () => window.location.href, navigate = (url) => window.location.assign(url) }: WorkspaceAppProps) {
  ...
  useEffect(() => {
    ...
    void workspaceRepository.load().then((saved) => {
      if (!cancelled && saved !== null) dispatch({ type: 'workspace/replace', snapshot: saved });
  ...
  const surfaceContent = activeTab?.kind === 'graph' ? (
    <section className="workspace-home"><strong>Graph</strong><p>Graph navigation will project the canonical local workspace in Iteration 7.</p></section>
    ...
    <section className="workspace-home"><strong>{activeChat.label}</strong><p>{activeChat.target}</p><button type="button" onClick={() => openSavedChat(activeChat)}>Open in ChatGPT</button></section>
  ) : (
    <section className="workspace-home"><strong>{workspace.name}</strong><p>{status}</p><dl className="workspace-stats"><div><dt>Folders</dt><dd>{workspace.folders.length}</dd></div><div><dt>Chats</dt><dd>{workspace.chatRefs.length}</dd></div><div><dt>Notes</dt><dd>{workspace.notes.length}</dd></div></dl></section>
  );
